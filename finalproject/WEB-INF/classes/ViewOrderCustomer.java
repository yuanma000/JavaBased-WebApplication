...
public class ViewOrderCustomer extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		try{
			all = MySqlDataStoreUtilities.getOrdersC(userName);
		}catch(Exception e){
		...
		pw.print("<h2>Your Car Rental Orders</h2><div class='entry'><table class='table'>");
		int i = 1;
		int size = hm.size();
		...
		for (Map.Entry<Integer, Order> entry : hm.entrySet()) {
			...
			{
				pw.print("<form name ='ViewOrderCustomer' action='ViewOrderCustomer' method='get'>");
		...
		if(request.getParameter("Order")!=null && request.getParameter("Order").equals("Cancel"))
		{
			if(request.getParameter("id") != null)
			{
				...
				try
				{
					...
					pw.print("<h4 style='color:red'>Your order is canceled</h4>");
					response.setHeader("Refresh", "1; URL=http://localhost/finalproject/ViewOrderCustomer");
				}
				catch(Exception e)
				{
